...
def get_table(volume_ratio_cutoff, df=companies):
    rsi_volume = pd.DataFrame([get_latest_volume_ratio(ticker) for ticker in yahoo_tickers.tickers],
                              columns=['vol_ratio', 'ratio_date']).reindex(df.index)
    df = pd.concat([df, rsi_volume], axis=1)
    df['action'] = df.vol_ratio.apply(lambda x: 'BUY/SELL' if x > volume_ratio_cutoff else np.nan)
    max_date =  max(df.ratio_date.dt.strftime('%Y-%m-%d'))
